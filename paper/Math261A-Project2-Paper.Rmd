...
This project analyzes San Francisco’s 2018–2019 Street & Sidewalk Cleaning (SSC) 311 requests to see whether three readily available fields—request type, weekday, and time-of-day of opening—help explain how long a request remains open. After standard cleaning, I fit an OLS multiple linear regression with closure time in hours as the outcome. I summarized coefficient estimates with 95% confidence intervals and checked assumptions with Residuals vs Fitted and Normal Q–Q plots.

**Key findings**

The model’s explanatory power is minimal, with $R^2$ \(\approx\) 0.005; adj. $R^2$ \(\approx\) 0.001, the global F-test for all predictors is not significant, with p-value = 0.27, and most coefficient intervals cross zero. In plain terms, differences in mean closure time associated with request category, weekday, or time of day are small and imprecise relative to the overall spread. Diagnostics show heavy right tails and mild heteroskedasticity, reinforcing that large, infrequent delays control a large portion of variability. Taken together, the answer to the research question is basically "not much”. These three fields alone offer very limited ability to explain closure times. Operational factors not captured here, crew assignment, travel distance, or backlogs likely drive most of the variation and would be more promising targets for analysis or process improvement.
